...
T.ItemDelegate {
    ...
    property int separatorHeight: MosSingleton.lineSvg.horLineHeight + (2 * Kirigami.Units.smallSpacing)
    ...
    function openActionMenu(x = undefined, y = undefined) {
        ...
        if (actions && actions.length > 0) {
            MosActionMenu.plasmoid = mosLauncher;
            MosActionMenu.menu.visualParent = root;
            MosActionMenu.actionList = actions;
            if (x !== undefined && y !== undefined) {
                MosActionMenu.menu.open(x, y);
            } else {
                MosActionMenu.menu.openRelative();
            }
        }
    }
    ...
    spacing: MosSingleton.fontMetrics.descent
    ...
    action: T.Action {
        onTriggered: {
            ...
            if (root.view.model.trigger && root.view.model.trigger(root.index, "", null)) {
                if (mosLauncher.hideOnWindowDeactivate) {
                    mosLauncher.expanded = false;
                }
            }
        }
    }
    ...
    function performDrag(handler: DragHandler): void {
        if (!handler.active) {
            mosLauncher.dragSource.Drag.active = false;
            mosLauncher.dragSource.Drag.imageSource = "";
            mosLauncher.dragSource.sourceItem = null;
            return;
        }
        root.dragIconItem.grabToImage(result => {
            ...
            mosLauncher.dragSource.sourceItem = root;
            mosLauncher.dragSource.Drag.imageSource = result.url;
            mosLauncher.dragSource.Drag.mimeData = {
                "text/uri-list" : [root.url]
            };
            mosLauncher.dragSource.Drag.active = handler.active;
        });
    }
    ...
    MouseArea {
        ...
        hoverEnabled: root.view
            ...
            && mosLauncher.fullRepresentationItem && !mosLauncher.fullRepresentationItem.contentItem.busy && !mosLauncher.fullRepresentationItem.blockingHoverFocus
        ...
        onEntered: {
            ...
            if (!root.activeFocus && !mosLauncher.searchField?.activeFocus) {
    ...
    background: Loader {
        ...
        sourceComponent: PlasmaExtras.Highlight {
            ...
            active: root.view.activeFocus ||
                    (mosLauncher.contentArea === root && mosLauncher.searchField.activeFocus)
        }
    }
}
