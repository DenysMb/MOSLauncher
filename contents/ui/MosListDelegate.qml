...
AbstractMosItemDelegate {
    id: root

    property bool compact: Plasmoid.configuration.compactMode

    leftPadding: MosSingleton.listItemMetrics.margins.left
        + (mirrored ? MosSingleton.fontMetrics.descent : 0)
    rightPadding: MosSingleton.listItemMetrics.margins.right
        + (!mirrored ? MosSingleton.fontMetrics.descent : 0)
    ...
    contentItem: RowLayout {
        id: row
        spacing: MosSingleton.listItemMetrics.margins.left * 2
    ...
    Loader {
        ...
        sourceComponent: KSvg.SvgItem {
            width: parent.width
            height: MosSingleton.lineSvg.horLineHeight
