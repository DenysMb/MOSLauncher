...
BasePage {
    ...
    Connections {
        target: mosLauncher.rootModel.favoritesModel
        function onFavoriteAdded() : void {
            flashFavoriteAnimation.restart();
        }
    }

    sideBarComponent: MosListView {
        id: sideBar
        focus: true // needed for Loaders
        model: mosLauncher.rootModel
        isSidebar: true
        ...
        delegate: MosListDelegate {
            ...
            component FavoritesDropArea: DropArea {
                ...
                readonly property bool acceptableDrag: draggedItem && !mosLauncher.rootModel.favoritesModel.isFavorite(draggedItem.model.favoriteId)
            ...
            Loader {
                id: dropAreaLoader
                anchors.fill: parent
                active: sideBarDelegate.index === 0 && !!mosLauncher.dragSource.sourceItem
                sourceComponent: FavoritesDropArea {
                    ...
                    onDropped: event => {
                        if (acceptableDrag && !sideBarDelegate.ListView.isCurrentItem) {
                            mosLauncher.rootModel.favoritesModel.addFavorite(draggedItem.model.favoriteId, sideBar.model.favoritesModel.count)
                            event.accept(Qt.CopyAction)
                        } else {
                            event.accept(Qt.IgnoreAction)
                        }
    ...
    contentAreaComponent: VerticalStackView {
        ...
        readonly property Kicker.AppsModel appsModel: mosLauncher.rootModel.modelForRow(appsModelRow)
        Connections {
            target: mosLauncher.rootModel
        ...
        function showSectionView(sectionName: string, parentView: MosListView): void {
        ...
        Component {
            ...
            DropAreaListView {
                ...
                focus: true
                model: mosLauncher.rootModel.favoritesModel
            }
        }
        ...
        Connections {
            ...
            function onExpandedChanged() {
                if (!mosLauncher.expanded && mosLauncher.contentArea.currentItem) {
                    mosLauncher.contentArea.currentItem.forceActiveFocus()
                }
            }
        }
    }
