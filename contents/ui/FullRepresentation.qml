...
EmptyPage {
    ...
    // mosLauncher is main.qml
    leftPadding: -mosLauncher.backgroundMetrics.leftPadding
    rightPadding: -mosLauncher.backgroundMetrics.rightPadding
    topPadding: 0
    bottomPadding: -mosLauncher.backgroundMetrics.bottomPadding
    readonly property var appletInterface: mosLauncher
    ...
    contentItem: VerticalStackView {
        ...
        Component {
            ...
            MosListView {
                ...
                model: mosLauncher.runnerModel.count ? mosLauncher.runnerModel.modelForRow(0) : null
                delegate: MosListDelegate {
                ...
                Keys.onTabPressed: event => {
                    mosLauncher.firstHeaderItem.forceActiveFocus(Qt.TabFocusReason);
                }
                Keys.onBacktabPressed: event => {
                    mosLauncher.lastHeaderItem.forceActiveFocus(Qt.BacktabFocusReason);
                }
                ...
                T.StackView.onStatusChanged: {
                    if (T.StackView.status === T.StackView.Activating || T.StackView.status === T.StackView.Activating) {
                        mosLauncher.sideBar = null
                        mosLauncher.contentArea = searchView
                    }
                }
                ...
                Loader {
                    ...
                    sourceComponent: PlasmaExtras.PlaceholderMessage {
                        ...
                        Connections {
                            target: mosLauncher.runnerModel
        ...
        Keys.forwardTo: mosLauncher.searchField
    ...
    Loader {
        active: !!mosLauncher.dragSource.sourceItem
        anchors.fill: parent
        sourceComponent: DropArea {
            id: favoriteRemoveDropArea

            // should be  "as AbstractMosItemDelegate", but the type system gets confused when changing view style at runtime
            readonly property Item draggedItem: mosLauncher.dragSource.sourceItem
            ...
            onDropped: event => {
                if (draggedItem && mosLauncher.rootModel.favoritesModel.isFavorite(draggedItem.model.favoriteId) && draggedItem.view.model instanceof Kicker.KAStatsFavoritesModel) {
                    mosLauncher.rootModel.favoritesModel.removeFavorite(draggedItem.model.favoriteId);
                    event.accept(Qt.MoveAction)
                } else {
