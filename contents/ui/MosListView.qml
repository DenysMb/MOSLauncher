    ...
    contentItem: ListView {
        ...
        implicitWidth: {
            ...
            if (root.mainContentView) {
                ...
                return MosSingleton.gridCellSize * mosLauncher.minimumGridRowCount + totalMargins
            }
            return contentWidth + totalMargins
        }
        implicitHeight: {
            // use grid cells to determine size
            let h = MosSingleton.gridCellSize * mosLauncher.minimumGridRowCount
            ...
            if (Plasmoid.configuration.favoritesDisplay !== 0 && Plasmoid.configuration.applicationsDisplay !== 0) {
                h = Math.floor(h / mosLauncher.listDelegateHeight) * mosLauncher.listDelegateHeight
            }
        ...
        section {
            ...
            delegate: PlasmaExtras.ListSectionHeader {
                ...
                width: view.availableWidth
                height: MosSingleton.compactListDelegateHeight
        ...
        onCurrentItemChanged: {
            if (launchMatchTimer.running && view.currentItem?.text.toLowerCase().includes(mosLauncher.runnerModel.query.toLowerCase())) {
                launchMatchTimer.stop()
        ...
        Keys.onPressed: event => {
            ...
            if (count >= 1 || (mosLauncher.runnerModel.querying && [Qt.Key_Return, Qt.Key_Enter].includes(event.key))) {
                switch (event.key) {
                    case Qt.Key_Up: if (!atFirst) {
                        decrementCurrentIndex()

                        if ((currentItem as AbstractMosItemDelegate)?.isSeparator) {
                            decrementCurrentIndex()
                        }
                    ...
                    case Qt.Key_Enter:
                        if (launchMatchTimer.running) {
                            launchMatchTimer.stop()
                            launchMatchTimer.triggered()
                        } else if (!mosLauncher.runnerModel.querying || view.currentItem?.text.toLowerCase().includes(mosLauncher.runnerModel.query.toLowerCase())) {
                            launchMatchTimer.triggered()
                        } else {
