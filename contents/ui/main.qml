...
PlasmoidItem {
    ...
    // instance of MOS Launcher requires different instances of these properties
    ...
    readonly property bool mayHaveGridWithScrollBar: Plasmoid.configuration.applicationsDisplay === 0
        || (Plasmoid.configuration.favoritesDisplay === 0 && mosLauncher.rootModel.favoritesModel.count > minimumGridRowCount * minimumGridRowCount)
    ...
    readonly property Kicker.RootModel rootModel: Kicker.RootModel {
        ...
        Component.onCompleted: {
            favoritesModel.initForClient("io.github.DenysMb.MOSLauncher.favorites.instance-" + Plasmoid.id)
    ...
    readonly property Kicker.RunnerModel runnerModel: Kicker.RunnerModel {
        query: mosLauncher.searchField ? mosLauncher.searchField.text : ""
        onRequestUpdateQuery: query => {
            if (mosLauncher.searchField) {
                mosLauncher.searchField.text = query;
            }
        }
        appletInterface: mosLauncher
        mergeResults: true
        favoritesModel: mosLauncher.rootModel.favoritesModel
    }
    ...
    // Used to show a smaller launcher on small screens
    readonly property int minimumGridRowCount: Math.min(Screen.desktopAvailableWidth, Screen.desktopAvailableHeight) * Screen.devicePixelRatio < MosSingleton.gridCellSize * 4 + (fullRepresentationItem ? fullRepresentationItem.normalPage.preferredSideBarWidth : MosSingleton.gridCellSize * 2) ? 2 : 4
    //END
    ...
    compactRepresentation: MouseArea {
        ...
        readonly property var sizing: {
            ...
            if (mosLauncher.inPanel) {
                if (mosLauncher.vertical) {
                    return {
        ...
        onPressed: wasExpanded = mosLauncher.expanded
        onClicked: mosLauncher.expanded = !wasExpanded
        ...
        Timer {
            ...
            interval: 250
            onTriggered: mosLauncher.expanded = true
        }
        ...
        RowLayout {
            ...
            Kirigami.Icon {
                ...
                Layout.fillWidth: mosLauncher.vertical
                Layout.fillHeight: !mosLauncher.vertical
                Layout.preferredWidth: mosLauncher.vertical ? -1 : height / (implicitHeight / implicitWidth)
                Layout.preferredHeight: !mosLauncher.vertical ? -1 : width * (implicitHeight / implicitWidth)
            ...
            Kirigami.Icon {
                ...
                Layout.fillWidth: mosLauncher.vertical
                Layout.fillHeight: !mosLauncher.vertical
                Layout.preferredWidth: mosLauncher.vertical ? -1 : height
                Layout.preferredHeight: !mosLauncher.vertical ? -1 : width
                Layout.alignment: Qt.AlignVCenter | Qt.AlignHCenter
            ...
            Image {
                ...
                Layout.preferredWidth: mosLauncher.vertical ? -1 : height / (implicitHeight / implicitWidth)
                Layout.preferredHeight: !mosLauncher.vertical ? -1 : width * (implicitHeight / implicitWidth)
                Layout.maximumHeight: mosLauncher.vertical ? -1 : Kirigami.Units.iconSizes.huge
                Layout.maximumWidth: mosLauncher.vertical ? Kirigami.Units.iconSizes.huge : -1
                Layout.alignment: Qt.AlignVCenter | Qt.AlignHCenter
