...
DropArea {
    ...
    // DropArea, which allows to drag and drop items outside mosLauncher
    // without changing their order within the view.
    property var dragMoves: []

    onPositionChanged: drag => {
        if (drag.source === mosLauncher.dragSource) {
            const source = mosLauncher.dragSource.sourceItem
            if (source === null) {
                return
            }
    ...
    onDropped: drag => {
        if ((drag.source !== mosLauncher.dragSource || mosLauncher.dragSource.sourceItem === null) && drag.hasUrls) {
            const pos = mapToItem(view.contentItem, drag.x, drag.y)
