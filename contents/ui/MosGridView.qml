...
EmptyPage {
    ...
    header: MouseArea {
        implicitHeight: MosSingleton.listItemMetrics.margins.top
        hoverEnabled: true
        onEntered: {
            if (containsMouse) {
    ...
    footer: MouseArea {
        implicitHeight: MosSingleton.listItemMetrics.margins.bottom
        hoverEnabled: true
        onEntered: {
            if (containsMouse) {
    ...
    GridView {
        ...
        anchors.horizontalCenterOffset: if (mosLauncher.mayHaveGridWithScrollBar) {
            if (root.mirrored) {
        ...
        width: Math.min(parent.width, Math.floor((parent.width - leftMargin - rightMargin - (mosLauncher.mayHaveGridWithScrollBar ? verticalScrollBar.implicitWidth : 0)) / cellWidth) * cellWidth + leftMargin + rightMargin)

        states: [
            State {
                name: "centered"
                when: !mosLauncher.mayHaveGridWithScrollBar
                AnchorChanges {
                    target: view
        ...
        implicitWidth: {
            let w = view.cellWidth * mosLauncher.minimumGridRowCount + leftMargin + rightMargin
            if (mosLauncher.mayHaveGridWithScrollBar) {
                w += verticalScrollBar.implicitWidth
            }
            return w
        }
        implicitHeight: view.cellHeight * mosLauncher.minimumGridRowCount + topMargin + bottomMargin

        leftMargin: mosLauncher.backgroundMetrics.leftPadding
        rightMargin: mosLauncher.backgroundMetrics.rightPadding

        cellHeight: MosSingleton.gridCellSize
        cellWidth: MosSingleton.gridCellSize
        ...
        Keys.onMenuPressed: event => {
            const delegate = currentItem as AbstractMosItemDelegate;
            if (delegate !== null) {
        ...
        Keys.onPressed: event => {
            ...
            if (count > 0 && (event.key == Qt.Key_Return || event.key == Qt.Key_Enter)) {
                (root.currentItem as AbstractMosItemDelegate).action.triggered();
                root.currentItem.forceActiveFocus(Qt.ShortcutFocusReason);
                event.accepted = true;
            } else if (count > 1) {
                switch (event.key) {
                    case Qt.Key_Left: if (!atLeft && !mosLauncher.searchField.activeFocus) {
                        moveCurrentIndexLeft()
                        focusCurrentItem(event, Qt.BacktabFocusReason)
                    } break
                    case Qt.Key_H: if (!atLeft && !mosLauncher.searchField.activeFocus && event.modifiers & Qt.ControlModifier) {
                        moveCurrentIndexLeft()
                        focusCurrentItem(event, Qt.BacktabFocusReason)
                    } break
                    ...
                    case Qt.Key_Right: if (!atRight && !mosLauncher.searchField.activeFocus) {
                        moveCurrentIndexRight()
                        focusCurrentItem(event, Qt.TabFocusReason)
                    } break
                    case Qt.Key_L: if (!atRight && !mosLauncher.searchField.activeFocus && event.modifiers & Qt.ControlModifier) {
                        moveCurrentIndexRight()
                        focusCurrentItem(event, Qt.TabFocusReason)
                    } break
